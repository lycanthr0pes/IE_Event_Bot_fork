"""書戻し失敗からの復旧・重複拒否・途中回収。APIはローカル代替。"""
...
class WritebackScenario(Scenario):
    rejections: int = 0


def scenario(monkeypatch, status=400, code="validation_error"):
    test = WritebackScenario(monkeypatch)
    ...
    async def api(url, options):
        if json.loads(options.get("body") or "{}").get("properties", {}).get("メッセージID", {}).get("rich_text") == "invalid":
            test.rejections += 1
...
def advance(test, count):
    status, payload = test.call("notion-writeback")
...
def test_actual_writeback_response_retains_queue_then_recovers(monkeypatch):
    ...
    failed_slot = next(s for s in test.owner()["fixtures"] if s["google_event_id"] == pending_id)
    assert probe._property_text(test.pages[failed_slot["notion_page_id"]], "メッセージID", "rich_text") == ""
    ...
    assert all(pair == ids[i] for i, pair in enumerate(partial_ids) if pair[0])
    assert test.call("advance")[0] == 200
    assert test.call("verify")[0] == 200
    ...
    assert stages["notion_writeback_api_rejection"] == 400
    assert stages["notion_writeback_failed_dispatch"] == 500
    assert all(stages[f"notion_writeback_{stage}"] == 200 for stage in (
        "validation_error", "cursor_preserved", "queue_only_retry", "reapply",
        "partial_maps", "writeback_missing", "same_ids",
        *[f"step_{i}" for i in range(4)], *[f"unique_{i}" for i in range(4)],
    ))
    assert not any(k.startswith("notion_query_") for k in stages)
    assert all(test.owner()["stages"][f"notion_writeback_cleanup_{i}"] == 200 for i in range(3))
...
def test_cannot_remove_writeback_failure_requirement(monkeypatch):
    test = scenario(monkeypatch)
    advance(test, 0)
    owner = test.owner()
    owner["notion_writeback_retry"] = False
...
@pytest.mark.parametrize("mode", ["notion_query_retry", "notion_create_retry", "matrix", "all_sync", "boundary"])
def test_cannot_mix_retry_modes(monkeypatch, mode):
    test = scenario(monkeypatch)
    advance(test, 0)
    owner = test.owner()
    owner[mode] = True
...
@pytest.mark.parametrize("key", [KEYS[1], KEYS[2], KEYS[3]])
def test_partial_state_tampering_prevents_retry(monkeypatch, key):
    test = scenario(monkeypatch)
    advance(test, 1)
    test.env.STATE_KV.data[key] = "[]" if key == KEYS[3] else "{}"
    assert test.call("advance")[0] == 409
    assert len(test.pages) == len(test.discord) == 2


@pytest.mark.parametrize("step", [1, 2])
def test_wrong_writeback_value_prevents_verification(monkeypatch, step):
    test = scenario(monkeypatch)
    advance(test, step)
    slot = next(s for s in test.owner()["fixtures"] if s.get("notion_page_id"))
    test.pages[slot["notion_page_id"]]["properties"]["メッセージID"] = {"rich_text": [{"text": {"content": "wrong-id"}}]}
    assert test.call("verify")[0] == 409
    assert test.call("cleanup")[0] == 200
    assert test.owner()["outcome"] == "failed_clean"
